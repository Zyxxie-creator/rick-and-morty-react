  ...
  useEffect(() => {
    const fetchEpisodes = async () => {
      ...
      try {
        ...
        const episodeRequests = episodesIds.map((id) =>
          axios.get(`${API_EPISODES_URL}/${id}`)
        );

        const results = await Promise.allSettled(episodeRequests);
        const fetchedSeries = results
